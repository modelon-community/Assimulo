...
absrel=np.ones((4,),dtype=np.float32)
wght=np.ones((4,),dtype=np.float32)

#...SINGULAR PERTURBATION PARAMETER
eps = np.array([.33333333e-3],np.float32)

#...SIMULATION OF BELOUSOV-ZHABOTINSKII REACTION

a = np.empty((m,m),dtype=np.float32)
w = np.empty((m,m),dtype=np.float32)
slu= np.empty((2*m,),dtype=np.float32)
