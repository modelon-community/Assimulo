...
C  or user supplied subroutine EMJAC (see constant QJACUS)
C
...
C                       (H .EQ. 0. ,if EULSIM fails to proceed)
...
C
     ...
     4NSTMAX
C
      DOUBLE PRECISION A, ALPHA, ANORM1, AWK, B1, C, CKAP, CKQ, CMY,
     2CMYH, CMYMAX, COSTF, COSTJ, COSTLR, COSTS, CMYRED, D, DABS, DBLE,
     3DEL, DELS, DELT0, DELT0A, DMAX1, DMIN1, DMY, DMT, DSQRT, DT,
     4DY, DZ, EMERRN, EPDIFF, EPKAP, EPMACH, TOLMIN, EPSAFE, ERR, ETA,
     5ETAD, ETADIF, ETAMIN, ETAMAX, FC, FCK, FCM, FCO, FMIN, FNJ, FNJ1,
     6FN1, FN, H, HALF, HJ, HL, HMAX, HMAXMY, HMAXU, HN, HREST, HRTRN,
     7OMJ, OMJO, ONE, PCT1, QQ, RED, RMAX, RO, ROW, SAFE, SAFEIN, SMALL,
     8SUMD, T, TEND, THETA, THMIN, THOPT, TN, TOL, TOLH, TOLN, TWO, U,
     9V, W, WY, WZ, Y, YA, YM, YWGT, YWGTN, ZERO
C
...
C    EMSCAL        EXT  Subroutine EMSCAL (IFLAG, Y, N, YWGT, THRESH)
C                       Scaling for EULSIM
...
C    EMERRN        EXT  Double precision function EMERRN(Y, N, YWGT)
...
C
     ...
     2 DEL(MAXODE), DELS(MAXODE), DT(MAXODE,JM), DY(MAXODE), DZ(MAXODE),
     3 ETA(MAXODE), FCK(KM), IPIVOT(MAXODE), NJ(JM), QQ(MAXODE,MAXODE),
...
C---  Compute amount of work per row of extrapolation tableau
      ...
      DO 101 J=2,JM
         ...
         AWK(J) = V
         DO 1011 K=1,J1
 1011       D(J,K) = (FNJ / DBLE(NJ(K)))
C        ENDDO
         IF (J .NE. 2) THEN
            W = V - AWK(1) + ONE
            DO 1012 K1=2,J1
               K = K1 - 1
               U = (AWK(K1) - V) / (W*DBLE(K1))
               U = TOLH**U
 1012          ALPHA(J1,K) = U
C           ENDDO
         ENDIF
...
C---1.1 Evaluation of cost coefficients
      ...
      IF ((COSTS + COSTLR + COSTJ) .NE. ZERO) THEN
         AWK(1) = COSTJ + COSTLR + (COSTF + COSTS)*FN1
         DO 11 J=2,JMACT
            J1 = J - 1
 11         AWK(J) = AWK(J1) +
     2         (COSTF + COSTS) * (DBLE(NJ(J)) - ONE) +
     3         TWO*COSTS + COSTLR
C        ENDDO
      ENDIF
C
...
C               AWK(JOPT+1)*PCT1 .LE. AWK(JOPT)*ALPHA(JOPT,KOPT))
         IF (JOPT .GE. JMACT)                                  GOTO 122
         IF (AWK(JOPT+1)*PCT1 .GT. AWK(JOPT)*ALPHA(JOPT,KOPT)) GOTO 122
C                                                              EXIT 121
         KOPT = JOPT
         JOPT = JOPT + 1
         GOTO  121
...
C                       EXIT 3.1 to stepsize reduction
               ...
               ENDIF
C              *************************************************
C---           Preparation for Newton restriction
               IF (NJ(J) .EQ. 2 .AND. QDM) THEN
                  DO 3113 I = 1,N
 3113                DELS(I) = DEL(I)
C                 ENDDO
                  CALL DGESL (QQ, MAXODE, N, IPIVOT, DELS, JOB)
                  NSOL = NSOL + 1
               ENDIF
C              *************************************************
C
C---3.1.3      Further semi-implicit Euler steps
               DO 313 K=1,M1
                  CALL  FCN (N, T + HJ*DBLE(K), YM, DEL, IFAIL)
                  NFCN = NFCN + 1
                  DO 3132 I=1,N
 3132                DEL(I) = HJ*DEL(I) / YWGT(I)
C                 ENDDO
                  CALL DGESL (QQ, MAXODE, N, IPIVOT, DEL, JOB)
                  NSOL = NSOL + 1
                  DO 3133 I=1,N
 3133                YM(I) = YM(I) + DEL(I)*YWGT(I)
C                 ENDDO
C
C                 *************************************************
C---              Newton restriction
C---              THETA .LE. THOPT*TWO
                  IF (K .EQ. 1 .AND. NJ(J) .EQ. 2 .AND. QDM) THEN
                     DMT = ZERO
                     DO 3134 I = 1,N
                       DY(I) = DEL(I) - DELS(I)
 3134                  DMT = DMT + DY(I)*DY(I)
C                    ENDDO
                     THETA = DSQRT(DMT / DMY)
...
C---3.1.6         Convergence monitor
 ...
 3161                YWGTN(I) = YWGT(I)
C                 ENDDO
                  CALL EMSCAL (1, YM, N, YWGTN, EPSAFE)
                  ERR = EMERRN (DY, N, YWGTN)
                  QKONV = ERR .LE. TOL
                  ERR = ERR / TOLH
C
C---              Order control
...
C---2.7.1 Stepsize restrictions
         ...
         IF (QMY) THEN
            ...
            ENDIF
         ENDIF
         HMAX = DMIN1(HMAXU,HMAXMY,H/FMIN)
         FCM = H / HMAX
C
...
C
      END
      DOUBLE PRECISION FUNCTION EMERRN(Y, N, YWGT)
C
C  Scaled root mean square error
C
C
...
C     ENDDO
      EMERRN = DSQRT(SUM / DBLE(N))
      RETURN
      END
      SUBROUTINE EMJAC (M, N, T, Y, A)
C
C     Provide Jacobian matrix A = DF/DY if desired
...
C
      WRITE (6, '(A)')
     2   ' +++ ERROR +++ EMJAC should not have been called.',
     3   '               Provide routine which computes Jacobian'
C
      RETURN
      END
      SUBROUTINE EMSCAL (IFLAG, Y, N, YWGT, THRESH)
C
C  Standard scaling
...
C        ENDDO
      ...
      RETURN
      END
      SUBROUTINE EMSEQ(M,NJ)
C
C  Set stepsize sequence for semi-implicit Euler method
C
      INTEGER M, NJ, I
      DIMENSION NJ(M)
      NJ(1) = 1
